...
def signin(): # 用户登录
    ...
    if request.method == 'GET':
        ...
        user_form = request.form #获取请求表单
        u = User.query.filter_by(email=user_form["email"]) # 查找所有用户
        user = u.first() # 选取登录的用户
        if user and user.verify_password(user_form["password"]): # 验证用户密码
            login_user(user) # 登录用户
            user.last_login = datetime.now() # 记录用户登录时间
            db.session.commit() # 提交数据库会话
            return redirect(request.args.get("next") or url_for("voice.index")) # 用户成功登录，返回主界面
        else:
            msg = gettext("Invalid username or password.") # 获取
            return render_template(tample_url, form=user_form, title=gettext("user sign in"), message=msg) # 返回重新登录帐号密码界面
