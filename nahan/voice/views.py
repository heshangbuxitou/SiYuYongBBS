...
def index():
    ...
    pagination = Pagination(page=page, total=Topic.query.filter_by(deleted=False).count(),
                            per_page=per_page,
                            record_name='topics',
...
def view(tid):
    ...
    # Save the comment and update the topic view page.
        ...
        if not current_user.is_authenticated or current_user._get_current_object().deleted:
            abort(403)
        ...
        if _file.filename != '':
            ...
            upload_filename = str(current_user.id) + '_' + _file.filename
            if _file and '.' in _file.filename and file_appendix in allowed_extensions:
                _file.save(os.path.join(upload_folder, upload_filename))
                reply_content += "\n![]({})".format(url_for('static',filename='upload/%s' % upload_filename))
...
def create():
    ...
    if request.method == 'GET':
        ...
        node_content = request.form['content']
        user_node_id = int(request.form['node'])
        user_id = current_user.id

        user_new_topic = Topic(user_id, title, node_content, user_node_id) # 创建新的话题
        user_new_topic.content_rendered = add_user_links_in_content(user_new_topic.content_rendered)#将话题中的内容转义
        db.session.add(user_new_topic)# 添加数据到数据库
        db.session.commit()# 提交数据到数据库
        user_topic_id = user_new_topic.id # 获取id

        # Generate notify from the topic content.
        add_notify_in_content(user_new_topic.content, current_user.id, user_topic_id) #解析内容，添加提醒到数据库

        # Update the user's and node's topics
        current_user.add_topic(user_topic_id) #更新已经登录的用户信息
        Node.query.filter_by(id=user_node_id).first().add_topic(user_topic_id)#添加User_id
        db.session.commit()

        return redirect(url_for('voice.view', tid=user_topic_id))
    else:
        abort(404)
...
def node_view(nid):
    ...
    pagination = Pagination(page=page,
                            total=Topic.query.filter_by(node_id=nid, deleted=False).count(),
                            per_page=per_page,
                            record_name='topics',
                            CSS_FRAMEWORK='bootstrap',
                            bs_version=3)
